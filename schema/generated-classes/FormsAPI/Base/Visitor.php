...
abstract class Visitor implements ActiveRecordInterface
{
    ...
    public function setUWStudentNumber($v)
    {
        ...
        if (!is_resource($v) && $v !== null) {
            $this->uw_student_number = fopen('php://memory', 'r+');
            fwrite($this->uw_student_number, $v);
            rewind($this->uw_student_number);
    ...
    public function setUWNetID($v)
    {
        ...
        // of the stream itself may have changed externally.
        if (!is_resource($v) && $v !== null) {
            $this->uw_net_id = fopen('php://memory', 'r+');
            fwrite($this->uw_net_id, $v);
            rewind($this->uw_net_id);
        } else { // it's already a stream
            $this->uw_net_id = $v;
        }
    ...
    public function setFirstName($v)
    {
        ...
        // of the stream itself may have changed externally.
        if (!is_resource($v) && $v !== null) {
            $this->first_name = fopen('php://memory', 'r+');
            fwrite($this->first_name, $v);
            rewind($this->first_name);
        } else { // it's already a stream
            $this->first_name = $v;
        }
    ...
    public function setMiddleName($v)
    {
        ...
        // of the stream itself may have changed externally.
        if (!is_resource($v) && $v !== null) {
            $this->middle_name = fopen('php://memory', 'r+');
            fwrite($this->middle_name, $v);
            rewind($this->middle_name);
        } else { // it's already a stream
            $this->middle_name = $v;
        }
    ...
    public function setLastName($v)
    {
        ...
        // of the stream itself may have changed externally.
        if (!is_resource($v) && $v !== null) {
            $this->last_name = fopen('php://memory', 'r+');
            fwrite($this->last_name, $v);
            rewind($this->last_name);
        } else { // it's already a stream
            $this->last_name = $v;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            if (null !== $col) {
                $this->uw_student_number = fopen('php://memory', 'r+');
                fwrite($this->uw_student_number, $col);
                rewind($this->uw_student_number);
            } else {
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->isNew() || $this->isModified()) {
                ...
                // Rewind the uw_student_number LOB column, since PDO does not rewind after inserting value.
                if ($this->uw_student_number !== null && is_resource($this->uw_student_number)) {
                    rewind($this->uw_student_number);
                }

                // Rewind the uw_net_id LOB column, since PDO does not rewind after inserting value.
                if ($this->uw_net_id !== null && is_resource($this->uw_net_id)) {
                    rewind($this->uw_net_id);
                }

                // Rewind the first_name LOB column, since PDO does not rewind after inserting value.
                if ($this->first_name !== null && is_resource($this->first_name)) {
                    rewind($this->first_name);
                }

                // Rewind the middle_name LOB column, since PDO does not rewind after inserting value.
                if ($this->middle_name !== null && is_resource($this->middle_name)) {
                    rewind($this->middle_name);
                }

                // Rewind the last_name LOB column, since PDO does not rewind after inserting value.
                if ($this->last_name !== null && is_resource($this->last_name)) {
                    rewind($this->last_name);
                }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'uw_student_number':
                        if (is_resource($this->uw_student_number)) {
                            rewind($this->uw_student_number);
                        }
                        $stmt->bindValue($identifier, $this->uw_student_number, PDO::PARAM_LOB);
                        break;
                    case 'uw_net_id':
                        if (is_resource($this->uw_net_id)) {
                            rewind($this->uw_net_id);
                        }
                        $stmt->bindValue($identifier, $this->uw_net_id, PDO::PARAM_LOB);
                        break;
                    case 'first_name':
                        if (is_resource($this->first_name)) {
                            rewind($this->first_name);
                        }
                        $stmt->bindValue($identifier, $this->first_name, PDO::PARAM_LOB);
                        break;
                    case 'middle_name':
                        if (is_resource($this->middle_name)) {
                            rewind($this->middle_name);
                        }
                        $stmt->bindValue($identifier, $this->middle_name, PDO::PARAM_LOB);
                        break;
                    case 'last_name':
                        if (is_resource($this->last_name)) {
                            rewind($this->last_name);
                        }
                        $stmt->bindValue($identifier, $this->last_name, PDO::PARAM_LOB);
                        break;
                }
            }
            $stmt->execute();
