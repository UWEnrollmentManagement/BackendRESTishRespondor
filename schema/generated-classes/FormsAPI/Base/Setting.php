...
use \Exception;
use \PDO;
use FormsAPI\SettingQuery as ChildSettingQuery;
use FormsAPI\Map\SettingTableMap;
...
abstract class Setting implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\FormsAPI\\Map\\SettingTableMap';
    ...
    /**
     * Initializes internal state of FormsAPI\Base\Setting object.
     */
     ...
     * @return $this|\FormsAPI\Setting The current object (for fluent API support)
     */
    ...
    public function setKey($v)
    {
        ...
        if ($this->key !== $v) {
            $this->key = $v;
            $this->modifiedColumns[SettingTableMap::COL_KEY] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SettingTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SettingTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = SettingTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\FormsAPI\\Setting'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SettingTableMap::COL_KEY)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\FormsAPI\Setting The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SettingTableMap::COL_KEY)) {
            $criteria->add(SettingTableMap::COL_KEY, $this->key);
        }
        if ($this->isColumnModified(SettingTableMap::COL_VALUE)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \FormsAPI\Setting (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \FormsAPI\Setting Clone of current object.
     * @throws PropelException
     */
