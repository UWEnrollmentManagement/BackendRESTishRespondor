...
class SubmissionTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'FormsAPI.Map.SubmissionTableMap';
     ...
     */
    const OM_CLASS = '\\FormsAPI\\Submission';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'FormsAPI.Submission';
     ...
     */
    const COL_FORM_ID = 'submission.form_id';

    /**
     * the column name for the status_id field
     */
    const COL_STATUS_ID = 'submission.status_id';

    /**
     * the column name for the assignee_id field
     */
    const COL_ASSIGNEE_ID = 'submission.assignee_id';
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\FormsAPI\\Submission');
  ...
  array (
    ...
    {
        return $withPrefix ? SubmissionTableMap::CLASS_DEFAULT : SubmissionTableMap::OM_CLASS;
    }
     ...
     * @return array           (Submission object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = SubmissionTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = SubmissionTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + SubmissionTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = SubmissionTableMap::OM_CLASS;
            /** @var Submission $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            SubmissionTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = SubmissionTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = SubmissionTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(SubmissionTableMap::DATABASE_NAME)->getTable(SubmissionTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(SubmissionTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(SubmissionTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new SubmissionTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Submission or Criteria object OR a primary key value.
     *
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \FormsAPI\Submission) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(SubmissionTableMap::DATABASE_NAME);
            $criteria->add(SubmissionTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = SubmissionQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            SubmissionTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                SubmissionTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return SubmissionQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Submission or Criteria object.
     *
     * @param mixed               $criteria Criteria or Submission object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Submission object
        }

        if ($criteria->containsKey(SubmissionTableMap::COL_ID) && $criteria->keyContainsValue(SubmissionTableMap::COL_ID) ) {
...
//
SubmissionTableMap::buildTableMap();
